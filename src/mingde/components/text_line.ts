...
export class TextLine<MessageType> implements Component<MessageType> {
  ...
  constructor(parent: WindowLike<MessageType | WindowMessage>, text: string, coords: [number, number], color: keyof ThemeInfo, font_size: keyof typeof FONT_SIZES, max_width: number | undefined) {
    this.parent = parent;
    this.text = text;
